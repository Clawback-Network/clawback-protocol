...
/**
 * Create and initialize a ClawBack XMTP agent.
 * Loads identity from config directory, creates XMTP signer, and initializes the agent.
 */
export async function createClawBackAgent(configDir?: string): Promise<Agent> {
  ...
  if (!walletKey) {
    throw new Error(
      "Wallet key not found. Run 'clawback identity' to generate a new identity.",
    );
  }
