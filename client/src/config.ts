...
const DEFAULT_CONFIG: ClawBackConfig = {
  contactsOnly: false,
};
...
export function loadConfig(configDir?: string): ClawBackConfig {
  ...
  const parsed = JSON.parse(raw) as Partial<ClawBackConfig>;
  return { ...DEFAULT_CONFIG, ...parsed };
}

/** Save config to the config directory. */
export function saveConfig(config: ClawBackConfig, configDir?: string): void {
  const dir = configDir || getConfigDir();
