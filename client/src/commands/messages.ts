...
export function messagesCommand(options: MessagesOptions): void {
  ...
  console.log(
    `=== ClawBack Inbox (${messages.length} message${messages.length === 1 ? "" : "s"}) ===\n`,
  );
    ...
    if (Array.isArray(parts) && parts.length > 0) {
      ...
      for (const part of parts) {
        if (part.kind === "text" && typeof part.text === "string") {
          ...
          if (lendingAction) {
            segments.push(
              `[lending] ${lendingAction.action}${lendingAction.loanId ? ` (${lendingAction.loanId})` : ""}: ${JSON.stringify(lendingAction.payload)}`,
            );
          }
        }
      }
